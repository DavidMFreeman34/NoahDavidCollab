...
r = max(p,q)
filename = 'pinwheel(%s,%s,%s)_spring.eps' %(p,q,m)
    ...
    for size,A,B,C in triangles:
        ...
        for i in xrange(l):
        	if round(size,5) == round(final_sizes[i],5):
        		tri_patch=PathPatch(tri,facecolor=cmap(0.5*(float(r)-i)/float(r)+i/float(r)),edgecolor='#000000',joinstyle='round',linewidth=0)
